...
{
    public sealed class GetTrashMediaAssetsHandler
    : IRequestHandler<GetTrashMediaAssetsQuery, PagedResult<MediaAssetListItemVm>>
    {
        private readonly IAppDbContext _db;
        public GetTrashMediaAssetsHandler(IAppDbContext db) => _db = db;

        public async Task<PagedResult<MediaAssetListItemVm>> Handle(GetTrashMediaAssetsQuery q, CancellationToken ct)
        {
            var qry = _db.MediaAssets
                .IgnoreQueryFilters()       // выключаем глобальный фильтр
                .Where(a => a.IsDeleted)    // показываем только корзину
                .AsNoTracking();
            ...
            if (q.DeletedFromUtc is not null)
                qry = qry.Where(a => a.DeletedAtUtc >= q.DeletedFromUtc);

            if (q.DeletedToUtc is not null)
                qry = qry.Where(a => a.DeletedAtUtc < q.DeletedToUtc);
